...
public final class Tcp {
    ...
    public static Thread serve(final int port, final Handler handler, final String name) {
        Thread t = new Thread(new Runnable() {
            public void run() {
                ExecutorService pool = Executors.newCachedThreadPool();
                try {
                    ServerSocket ss = new ServerSocket(port);
                    log(name + " escuchando en puerto " + port);
                    while (true) {
                        final Socket s = ss.accept();
                        pool.submit(new Runnable() { public void run() {
                            try { handler.handle(s); }
                            catch (Exception e) { log(name + " error cliente: " + e.getMessage()); }
                            finally { try { s.close(); } catch (IOException ignored) {} }
                        }});
                    }
                } catch (IOException e) {
                    log(name + " no pudo iniciar puerto " + port + ": " + e.getMessage());
                }
            }
        }, name + "-server");
        t.setDaemon(false);
    ...
    public static void log(String s) {
        System.out.println("[" + new java.text.SimpleDateFormat("HH:mm:ss.SSS").format(new java.util.Date()) + "] " + s);
    }
}
