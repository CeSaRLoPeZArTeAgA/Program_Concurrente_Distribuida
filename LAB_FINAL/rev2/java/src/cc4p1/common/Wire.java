...
/**
 * Protocolo de una línea: TYPE|k=v|k=v.
 * Los valores viajan codificados URL para evitar conflictos con separadores.
 */
public final class Wire {
    ...
    public static Map<String, String> parse(String line) {
        ...
        for (int i = 1; i < parts.length; i++) {
            ...
            if (eq <= 0) continue;
            out.put(dec(parts[i].substring(0, eq)), dec(parts[i].substring(eq + 1)));
        }
        return out;
    }
    ...
    public static int getInt(Map<String, String> m, String k, int def) {
        try { return Integer.parseInt(get(m, k, String.valueOf(def))); }
        catch (Exception e) { return def; }
    }

    public static long getLong(Map<String, String> m, String k, long def) {
        try { return Long.parseLong(get(m, k, String.valueOf(def))); }
        catch (Exception e) { return def; }
    }
